...
# ==========================================
# 🎁 究極完全版 v5.5: バグ絶対回避 ＋ 厳格な10%スタート ＋ 昭和アニメ名台詞
# ==========================================
...
def optimize_system(df, params):
    ...
    # 💡 強力なバグ対策: 空欄(NaN)や計算不能文字(inf)を強制的に「0.0」に変換
    ...
    for t in range(T):
        ...
        if t == 0:
            # 💡 厳格化：ズルを許さない！下限ギリギリの10%からスタートさせる
...
# 3. UI設定 (Streamlit)
# ==========================================
st.set_page_config(page_title="最適容量シミュレーター v5.5", layout="wide")
...
if demand_file:
    ...
    if len(df) > 0:
        if st.button("🚀 制約条件を守って最適解を計算", type="primary"):
            ...
            # 💡 遊び心1: ランダムで表示する昭和〜平成のレジェンド名台詞
            anime_quotes = [
                "見せてもらおうか、AIシミュレーターの性能とやらを！",
                "エネルギー充填120％！最適化エンジン、発進！",
                "あきらめたらそこで商談終了ですよ",
                "オラに元気を分けてくれ！（※太陽光で）",
                "燃え上がれ！俺のコスモ（計算エンジン）！",
                "パターン青、最適解です！"
            ...
            if results['Status'] == 'Infeasible':
                ...
                # 💡 遊び心3: 電気代の削減率が50%を超えた時の隠しメッセージ（ルパン風）
