...
# ==========================================
# 🎁 究極版 v4.5: テンプレート出力機能 + CSV出力 + 将来コスト考慮
# ==========================================
...
def optimize_system(df, params):
    ...
    opex_energy = Y * pulp.lpSum([
        (params['price_base'] + params['adj_fuel'] + params['surcharge']) * E_buy[t]
        for t in range(T)
    ])
...
# 3. UI設定 (Streamlit)
# ==========================================
st.set_page_config(page_title="最適容量シミュレーター v4.5", layout="wide")
...
if demand_file:
    ...
    if len(df) > 0:
        if st.button("🚀 シミュレーションを実行", type="primary"):
            with st.spinner('最適容量計算中... (糖分が足りないよ～( ;∀;))'):
            ...
            if results['Status'] == 'Optimal':
                ...
                st.error("最適解が見つかりませんでした。条件を見直してください。")