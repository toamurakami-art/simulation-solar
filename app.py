...
# ==========================================
# 🎁 究極完全版 v5.2: 全制約条件 ＋ リアル蓄電池ルール（グラフなし爆速版）
# ==========================================
...
def optimize_system(df, params):
    ...
    for t in range(T):
        ...
        # 💡 裏ルール1：実効容量の保護（10%残し、95%まで）
        prob += S[t] >= X_bat * 0.10
        prob += S[t] <= X_bat * 0.95

        # 💡 裏ルール2：出力の制限（出力kWは容量kWhの半分。30分での限界はそのさらに半分）
        max_kw = X_bat * 0.5
...
# 3. UI設定 (Streamlit)
# ==========================================
st.set_page_config(page_title="最適容量シミュレーター v5.2", layout="wide")
...
if demand_file:
    ...
    if len(df) > 0:
        if st.button("🚀 制約条件を守って最適解を計算", type="primary"):
            with st.spinner('最適解を計算中... (糖分が足りないよ～( ;∀;))'):
